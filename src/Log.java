...
public class Log {
    ...
    */
    public static void showlogs() throws Exception {
    	String head=Files.readString(Paths.get(publicPath+ File.separator+"HEAD"));
    	String branchname=head.substring(head.lastIndexOf("/")+1);//
    	String branchpath=publicPath+ File.separator+"Branch"+ File.separator+branchname;
    	File fl=new File(branchpath);
    	if(!fl.exists()) {
    		System.out.println("no commit yet,can not log");//没有commit时提示不可以log  		
    	}
    	else {
    		String curcommit=Files.readString(Paths.get(branchpath));   
	        String content="";
